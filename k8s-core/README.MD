...
vagrant@master-node:~$
```
As you can see, since the pods in the deployment does not have a toleration by default for the taint in worker-node01 all of them are deployed in worker-node02